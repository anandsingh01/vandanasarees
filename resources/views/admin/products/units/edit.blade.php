...
@section('body')
    ...
    <div class="container-fluid">
        <form class="form" action="{{url('products/update-units/'.$units->id)}}" method="post" enctype="multipart/form-data">

            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12">
                    <div class="card">
                        <div class="header">
                            <h2><strong>Update</strong> units</h2>
                        </div>
                        <div class="body">
                            <div class="row">
                                <div class="col-lg-12 col-md-12 mtb-10">
                                    <label class="control-label" for="password">units Title</label>
                                    <input type="text" class="form-control" name="unit_name" value="{{$units->unit_name}}" />
                                </div>
                                <div class="col-lg-12 col-md-12 mtb-10">
                                    <label class="control-label" for="Status">Status</label>
                                    <select class="form-control show-tick ms select2 removeSpace" name="is_active" data-placeholder="Select">
                                        <option value="active" {{$units->is_active == 'active' ? 'selected' : ''}}>Active</option>
                                        <option value="inactive" {{$units->is_active == 'inactive' ? 'selected' : ''}}>Inactive</option>
                                    </select>
                                </div>
                            </div>
                        </div>
