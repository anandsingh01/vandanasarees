...
<meta property="og:type"          content="{{$product->title}}" />
            ...
            <div class="row mb_100 product_description justify-content-lg-between">
                <div class="col-lg-6">
                    ...
                    <div id="productCarousel" class="carousel slide" data-bs-ride="carousel">
                        <div class="carousel-inner">
                            @foreach($product->getGallery as $key => $productdetails)
                                <div class="carousel-item {{ $key == 0 ? 'active' : '' }}">
                                    <img src="{{ asset($productdetails->image) }}" class="d-block w-100 carousel-img" alt="Product Image {{ $key + 1 }}">
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="row mt-2 text-center">
                        <div class="col-12 d-flex justify-content-center">
                            @foreach($product->getGallery as $key => $productdetails)
                                <div class="thumbnail mx-2" data-bs-target="#productCarousel" data-bs-slide-to="{{ $key }}">
                                    <img src="{{ asset($productdetails->image) }}" alt="Thumbnail {{ $key + 1 }}" class="img-fluid thumbnail-img">
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="col-lg-6">
                    <div class="shop_details_content">
                        <span class="f2p_code">Tpr: {{$product->product_sku_id}}</span>
                        <h2 class="item_title text-uppercase">{{$product->title}}</h2>
                        <hr>
                        <!-- Product Price and Savings Display -->
                        <span class="f2p_price mb_15">
    <strong style="font-size: 24px; color:green;">Rs. {{ number_format($product->product_actual_price, 2) }}</strong>
    <del>Rs. {{ number_format($product->product_max_selling_price, 2) }}</del>
    <br>
                            <!-- Display savings -->
    <span style="color: red; font-size: 18px;">
        You Save: Rs. {{ number_format($product->product_max_selling_price - $product->product_actual_price, 2) }}
        ({{ round((($product->product_max_selling_price - $product->product_actual_price) / $product->product_max_selling_price) * 100) }}%)
    </span>
</span>
                        ...
                        <div class="row">
                            ...
                            <div class="product-action">
                                <input type="number" id="quantityInput{{$product->id}}" value="1" min="1" max="3" class="quantity-input">
                                <a href="#"
                                   data-size="{{ $product->size }}"
                                   data-id="{{ $product->id }}"
                                   data-price="{{ $product->product_actual_price }}"
                                   data-product_name="{{ $product->title }}"
                                   data-qty="1"
                                   data-msp="{{ $product->product_max_selling_price }}"
                                   data-image="{{ $product->photo }}"
                                   data-variation_product_id="{{ $product->product_id }}"
                                   data-flash_sale="{{ $product->flash_sale }}"
                                   data-flash_price="{{ $product->flash_price }}"
                                   data-color_id_attr="{{ $product->getProductAttr[0]->id ?? '0' }}"
                                   class="btn-cart"
                                   id="qvid{{$product->id}}">
                        ...
                        <div class="accordion">
                            <div class="accordion-item">
                                ...
                                <div class="accordion-content">
                                    <img src="{{asset($productdetails->cert_of_auth)}}">
                                </div>
                            </div>
                ...
                @forelse($get_sarees as $key => $get_sareess )
                    <div class="product-item">
                        <a href="{{url('products/'.$get_sareess->slug)}}">
                            <img src="{{asset($get_sareess->photo)}}" alt="Beautiful Saffron Orange Lehenga With Heavy Hand Embroidery From Banaras">
                        </a>
                        <h3 class="product-title">
                            <a href="{{url('products/'.$get_sareess->slug)}}">    {{$get_sareess->title}} </a>
                        </h3>
                        <p class="product-price">
                            <span class="currency-symbol">₹</span> {{number_format($get_sareess->product_actual_price ?? '' , 2)}} <span
                                class="product-price-strike"><span class="currency-symbol">₹</span>  {{number_format($get_sareess->product_max_selling_price ?? '', 2)}}</span></p>
